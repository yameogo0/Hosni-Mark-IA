...
import { Send, User, Bot, Sparkles, AlertCircle, WalletIcon } from "lucide-react"
...
export default function ChatBot() {
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-background via-background to-primary/5 z-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full space-y-4 text-center">
          ...
          <div className="text-2xl font-bold text-foreground">{APP_CONFIG.NAME}</div>
          <div className="text-sm text-muted-foreground max-w-sm mx-auto leading-relaxed">
            Votre Expert en Marketing & Commerce 24/7
          </div>
          <div className={`text-base mt-6 ${error ? 'text-destructive' : 'text-foreground'}`}>{authMessage}</div>
          {!error && <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary mx-auto mt-4"></div>}
          {error && (
            <Button
              className="mt-6"
  ...
  // Check if this is the first conversation (only welcome message)
    ...
    <div className="flex items-center justify-center min-h-screen p-4" style={{ backgroundColor: COLORS.BACKGROUND }}>
      <Card className="w-full max-w-2xl h-[700px] flex flex-col shadow-2xl border-primary/20">
        ...
        <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
          {/* Show welcome content only on first load */}
          {isFirstMessage && (
          ...
          {/* Show subscription status in conversation */}
          {!isFirstMessage && (
          ...
          {messages.map((message) => {
            // Skip rendering the initial welcome message in the chat flow
        ...
        <CardFooter className="p-4 border-t bg-muted/30 space-y-3">
          {/* Question limit alert for free users */}
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedImage && (
                <span className="text-[10px] text-muted-foreground">
                  L&apos;image sera analysée avec votre question
                </span>
              )}
            </div>
          )}
          ...
          <div className="flex w-full gap-2">
            <Input
              ...
              disabled={isLoading || !subscriptionStatus.canAskQuestion}
          ...
          <div className="text-[10px] text-muted-foreground text-center w-full">
            Propulsé par Pi Network • Paiements sécurisés & décentralisés
            {" • "}
            <a href="/diagnostic" className="underline hover:text-foreground">
              Diagnostic
            </a>
          </div>
        </CardFooter>
